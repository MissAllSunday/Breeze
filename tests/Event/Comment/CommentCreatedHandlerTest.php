...
class CommentCreatedHandlerTest extends TestCase
{
	...
	private CommentCreatedHandler|MockObject $handler;
	...
	protected function setUp(): void
	{
		...
		$this->handler = $this->getMockBuilder(CommentCreatedHandler::class)
	...
	{
		$this->alertEntity->method('getContentAction')->willReturn('default');
		$this->alertEntity->method('getExtra')->willReturn(['status_id' => 123]);
		$this->alertEntity->method('toArray')->willReturn([]);
		$this->handler->method('global')->willReturn('http://example.com');
		$this->handler->method('parserText')->willReturn('url');
	...
	{
		$contentAction = EventAbstract::CONTENT_ACTION_CREATED . EventAbstract::WALL_OWNER;
	...
	{
		$contentAction = EventAbstract::CONTENT_ACTION_CREATED . EventAbstract::STATUS_OWNER;
