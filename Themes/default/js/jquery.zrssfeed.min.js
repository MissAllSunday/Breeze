...
var html='';var row='windowbg2';var xml=getXMLDocument(data.xmlString);var xmlEntries=xml.getElementsByTagName('item');if(options.header)
html+='<div class="rssHeader">'+'<a href="'+feeds.link+'" title="'+feeds.description+'">Live from Breeze support site:</a>'+'</div>';html+=''+'<ul>';for(var i=0;i<feeds.entries.length;i++){var entry=feeds.entries[i];var pubDate;if(entry.publishedDate){var entryDate=new Date(entry.publishedDate);var pubDate=entryDate.toLocaleDateString()+' '+entryDate.toLocaleTimeString();}
...
html+='</ul>'}
html+='</li>';}
if(row=='windowbg2'){row='even';}else{row='windowbg2';}}
html+='</ul>'+'</div>'
