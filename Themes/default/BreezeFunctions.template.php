...
function breeze_fetch_status($status)
{
	global $txt, $context;
		...
		<span class="topslice"><span></span></span>
			<div class="breeze_user_inner">
				...
				<div class="breeze_user_status_comment">
					...
					// Delete status
					if (!empty($delete_status))
						$return .= '| <a href="', $scripturl , '?action=breezeajax;sa=delete;bid=', $status['id'] ,';type=status;profile_owner=', $status['owner_id'] ,'', !empty($context['Breeze']['commingFrom']) ? ';rf='. $context['Breeze']['commingFrom'] : '' ,'" id="', $status['id'] ,'" class="breeze_delete_status">', $txt['Breeze_general_delete'] ,'</a>';
	...
	if (!empty($status['comments']))
		foreach($status['comments'] as $comment)
		{
			...
			if ($delete_comments == true || $delete_single_comment == true)
				$return .=
									'| <a href="', $scripturl , '?action=breezeajax;sa=delete;bid=', $comment['id'] ,';type=comment;profile_owner=', $status['owner_id'] ,'', !empty($context['Breeze']['commingFrom']) ? ';rf='. $context['Breeze']['commingFrom'] : '' ,'" id="', $comment['id'] ,'" class="breeze_delete_comment">', $txt['Breeze_general_delete'] ,'</a>';
		...
		// Display the new comments
		$return .= '<li id="breeze_load_image_comment_', $status['id'] ,'" style="margin:auto; text-align:center;"></li>';
		...
		if (!empty($post_comment))
		{
			...
			if (!empty($context['Breeze']['user_info'][$user_info['id']]['facebox']))
				$return .=
							$context['Breeze']['user_info'][$user_info['id']]['facebox'];
