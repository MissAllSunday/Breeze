...
const MiniProfile: React.FunctionComponent<MiniProfileProps> = (
	...
	const buddyIconClass = userData.is_buddy ? 'delete' : 'plus';
	const buddyTitle = userData.is_buddy ? smfTextVars.general.buddyRemove : smfTextVars.general.buddyAdd;
	const buddyUrl = `${SmfVars.script_url}?action=buddy;u=${userData.id};${SmfVars.session.var}=${SmfVars.session.id}`;
