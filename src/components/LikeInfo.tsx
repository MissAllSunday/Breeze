...
export const LikeInfo: React.FunctionComponent<LikeProps> = (
	props: LikeProps,
) => {
			...
			{Object.values(props.likeInfo.likes).map((like: LikeType) => (
				<li key={like.userData.id}>
					...
					<span className="like_profile">
						<span
							dangerouslySetInnerHTML={{ __html: like.userData.link_color }}
						/>
						<span className="description">{like.userData.group}</span>
					</span>
					<span
						className="like_time"
						dangerouslySetInnerHTML={{ __html: like.likeTime }}
					/>
				</li>
			))}
		</ul>
